...
@tree.command(name="dailydraw", description="ADMIN: Draw 1 Daily Lucky Star winner for @app_commands.checks.has_permissions(administrator=True)
...
@tree.command(name="finaldraw", description="ADMIN: Draw the Final Lucky Draw winners (only 7
@app_commands.checks.has_permissions(administrator=True)
...
index += 1
message = " **FINAL LUCKY DRAW WINNERS** await interaction.followup.send(message)
\n\n" + "\n".join(results)
